...
y1 <- students$shoesize
cor.test(x1, y1)
ggplot(students, aes(x = x1, y = y1)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red") #實際上繪製回歸線無意義
...
chisq.test(die)
# p-value = 0.03763 -> Cheating!

# 哈溫平衡
obs <- c(750, 50, 200)
exp <- c(0.60, 0.35, 0.05)
chisq.test (x=obs, p=exp) # p-value < 2.2e-16 -> Reject!

# frequency
F <- matrix(nrow=4,ncol=2,data=c(33,14, 8,18,31,25,14,12))
chisq.test(F) # alternative see `fisher.test`

# Student’s t-test: Several version exist
# 1 sample compared to a known mean
# 2 samples (with variance equal - Student’s or with unequal variance - Welsh’s)
# paired samples (compare two dependent (e.g. before-after) samples).
...
# A non-parametric alternative for the comparison of two samples:
# Mann-Whitney U-test (independent)
# Wilcoxon W-test (dependant)
...
# Kruskall-Wallis test (KW-test) is a non-parametric (assumption free) version of 1-way ANOVA.
